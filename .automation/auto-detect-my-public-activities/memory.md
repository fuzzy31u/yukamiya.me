...
- `https://developers.cyberagent.co.jp/blog/archives/55718/` (Technovation Girls TA) is attributed by search **to 清水 良一** of 技術広報, who was the TA himself. It surfaces on `site:...blog 神谷優` only because it is a Tech DE&I post and she leads that project. **A project mention is not an activity** — the same distinction as byline ≠ speaker. Parked in `activities` as `detected`, **not** in `rejected_candidates`: that bucket is only for non-matches verified from page metadata, and this one still owes a `<meta name="author">` read.
...
- **The fix is a configuration change in the routine's environment, and only the account owner can make it.** claude.ai/code → the routine → pencil **Edit routine** → the cloud icon under **Instructions** → the environment's settings icon → **Network access: Custom**, with the sweep's hosts in **Allowed domains** and *Also include default list of common package managers* checked. The host list is in the README. A run that still hits `403 host_not_allowed` should say so in one line and stop re-deriving it — it is not something the run can fix from inside the sandbox.
- Consequence while it is unfixed: the sweep can still run, but **source-page validation and link verification cannot**. In that state, never promote a candidate on search-summary evidence alone — record it as `detected` with the blocker and leave its validation to a run that can fetch. Leave `last_checked_at` stale for the author-page sources, and skip `QA Post Deploy` (its link findings would be unverifiable).
